...
import com.github.alexandrelombard.commonskt.math3.utils.MathUtils.equals
...
import com.github.alexandrelombard.commonskt.math3.utils.Precision.equalsWithRelativeTolerance
 ...
 * `x` is `NaN`). The method
 * [ equals for primitive double][org.apache.commons.math3.util.Precision.equals] in [org.apache.commons.math3.util.Precision]
...
class Complex constructor(
     ...
     * @return `this + addend`.
     */
    override fun add(addend: Complex): Complex {
 ...
 * prescaling of operands](http://doi.acm.org/10.1145/1039813.1039814) to limit the effects of overflows and
     ...
     * @see .multiply
     */
    override fun multiply(factor: Int): Complex {
        if (isNaN) {
            return NaN
        }
        return if (real.isInfinite() ||
     ...
     */
    override fun subtract(subtrahend: Complex): Complex {
 ...
 * square root](http://mathworld.wolfram.com/SquareRoot.html) of this complex number.
    ...
    fun sqrt(): Complex {
        ...
        val t = sqrt((abs(real) + abs()) / 2.0)
        return if (real >= 0.0) {
            createComplex(t, imaginary / (2.0 * t))
        } else {
            createComplex(
                abs(imaginary) / (2.0 * t),
                copySign(1.0, imaginary) * t
            )
        }
    }
 ...
 * tangent](http://mathworld.wolfram.com/Tangent.html) of this complex number.
    ...
    fun tan(): Complex {
        ...
        val d = cos(real2) + cosh(imaginary2)
        return createComplex(
            sin(real2) / d,
            sinh(imaginary2) / d
        )
 ...
 * hyperbolic tangent](http://mathworld.wolfram.com/HyperbolicTangent.html) of this complex number.
    ...
    fun tanh(): Complex {
        ...
        val real2 = 2.0 * real
        val imaginary2 = 2.0 * imaginary
        val d = cosh(real2) + cos(imaginary2)
        return createComplex(
            sinh(real2) / d,
            sin(imaginary2) / d
        )
     ...
     * @return a List of all `n`-th roots of `this`.
    ...
    fun nthRoot(n: Int): List<Complex> {
        ...
        val nthRootOfAbs = pow(abs(), 1.0 / n)

        // Compute nth roots of complex number with k = 0, 1, ... n-1
        val nthPhi = argument / n
        val slice = 2 * FastMath.PI / n
        var innerPart = nthPhi
        for (k in 0 until n) {
            // inner part
            val realPart = nthRootOfAbs * cos(innerPart)
            val imaginaryPart = nthRootOfAbs * sin(innerPart)
    ...
    /** {@inheritDoc}  */
    override val field: ComplexField
        get() = ComplexField.instance
